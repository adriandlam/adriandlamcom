...
The problem is that embedding models capture semantic meaning and miss exact keyword matches. Usually, embedding models are combined with BM25 (Best Matching 25).

## BM25: The Secret Sauce

BM25 (Best Matching 25) is a ranking function that uses lexical matching to find precise word or phrase matches. It's effective for queries that include unique identifiers or technical terms. For example, if a user queries "HTTP 200 Ok" in a technical support database, BM25 can find the exact "HTTP 200" match.
